...
export default function AdminTeacherClasses() {
  ...
  const loadEnrollments = useCallback(
    async (classId) => {
      ...
      try {
        ...
        toastActionError(err, 'Không tải được danh sách học viên trong lớp.')
        setEnrollments([])
      } finally {
  ...
  const submitNewClass = async (e) => {
    ...
    if (!classTeacher?.id || !newClass.name.trim()) {
      toast.warning('Chọn giáo viên và nhập tên lớp.')
      return
    }
    try {
      ...
      toastActionError(err, 'Không tạo được lớp.')
    }
  }
  ...
  const saveEditClass = async (e) => {
    ...
    try {
      ...
      toastActionError(err, 'Không lưu được thông tin lớp.')
    }
  }
  ...
  const pullClassToTeacher = async () => {
    ...
    if (!pullClassId || !Number.isFinite(id) || !classTeacher?.id) {
      toast.warning('Chọn một lớp để gán cho giáo viên này.')
      return
    }
    try {
      ...
      toastActionError(err, 'Không gán được lớp cho giáo viên.')
    }
  }
