...
import { postMyCoursePayment } from '../services/meApi.js'
import { useAuthModal } from '../context/AuthModalContext'
import { useAuthSession } from '../context/AuthSessionContext'

/**
 * Popup gửi yêu cầu thanh toán mua khóa học (theo giá niêm yết trên khóa).
 */
export default function CoursePaymentModal({
  ...
  const priceNum = listPrice != null && Number.isFinite(Number(listPrice)) ? Number(listPrice) : null
  ...
  const submitPayment = async (e) => {
    ...
    if (!hasListedPrice) {
      toast.error('Khóa này chưa có giá niêm yết trên hệ thống. Vui lòng liên hệ trung tâm.')
      return
    }
    ...
    setSubmitting(true)
    try {
      await postMyCoursePayment(session.access_token, {
      ...
      setPaymentSent(true)
      toast.success('Đã ghi nhận yêu cầu thanh toán. Sau khi được xác nhận, bạn sẽ mở được khóa học.')
      onSubmitted?.()
    } catch (e2) {
      if (import.meta.env.DEV) console.error('[CoursePaymentModal]', e2)
    ...
    if (!hasListedPrice) {
      return (
        <div className="space-y-3 text-sm text-gray-600 dark:text-slate-400">
          <p>
            Khóa này <strong className="text-gray-900 dark:text-slate-200">chưa có giá niêm yết</strong> trên hệ thống — không thể gửi yêu cầu thanh toán trực tuyến từ đây.
          </p>
      ...
      return (
        <div className="space-y-4">
          <p className="text-sm text-gray-600 dark:text-slate-400">
            Giá khóa: <span className="font-semibold text-gray-900 dark:text-slate-200">{priceLabel}</span>
          </p>
          <div className="rounded-xl border border-amber-200 bg-amber-50/90 px-4 py-3 text-sm text-amber-950 dark:border-amber-500/35 dark:bg-amber-950/35 dark:text-amber-100">
            ...
            <p className="mt-1.5 leading-relaxed">
              Để gửi yêu cầu thanh toán và mở khóa học sau khi được xác nhận, vui lòng{' '}
          ...
          <div className="flex flex-wrap gap-2">
            <button
              type="button"
              onClick={() => openLogin()}
              className="rounded-xl border border-cyan-500/40 bg-cyan-500/10 px-4 py-2.5 text-sm font-semibold text-cyan-800 dark:text-cyan-200"
            >
              Đăng nhập
            </button>
            <button
    ...
    if (user?.dbRole && user.dbRole !== 'student') {
      return (
        <p className="text-sm text-amber-800 dark:text-amber-200">
          Chỉ tài khoản học viên mới gửi được yêu cầu thanh toán khóa học. Vui lòng đăng nhập bằng tài khoản học viên.
        </p>
      )
    }
    ...
    if (paymentSent) {
      return (
        <p className="rounded-xl border border-emerald-500/30 bg-emerald-500/10 px-4 py-3 text-sm text-emerald-800 dark:text-emerald-200">
          Yêu cầu đã gửi. Trung tâm sẽ đối soát thanh toán; khi trạng thái là <strong>Đã thanh toán</strong>, bạn có thể vào xem khóa học ngay trên trang Khóa học.
        </p>
      )
    }
    ...
    return (
      <form onSubmit={submitPayment} className="grid gap-3">
        <div className="rounded-xl border border-gray-200 bg-gray-50 px-4 py-3 text-sm text-gray-800 dark:border-slate-600 dark:bg-slate-900/80 dark:text-slate-200">
          <p className="font-medium text-gray-900 dark:text-white">Mua khóa học trực tuyến</p>
          <p className="mt-1">
            Giá niêm yết: <span className="font-semibold text-cyan-700 dark:text-cyan-300">{priceLabel}</span>
          </p>
        </div>
        ...
        <button
          type="submit"
          disabled={submitting}
          className="rounded-xl bg-linear-to-r from-cyan-500 to-fuchsia-600 py-3 font-semibold text-white disabled:opacity-60"
        >
  ...
  return (
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="course-pay-title"
      onClick={(e) => {
    ...
    >
      ...
      >
        ...
        <h2 id="course-pay-title" className="pr-10 text-xl font-bold text-gray-900 dark:text-white">
          Thanh toán để mở khóa học
        </h2>
        <p className="mt-2 text-sm text-gray-600 dark:text-slate-400">
          Khóa: <span className="font-semibold text-gray-900 dark:text-slate-200">{courseTitle}</span>
        </p>
        <p className="mt-3 text-sm text-gray-600 dark:text-slate-400">
          Sau khi thanh toán được xác nhận, bạn có thể vào xem toàn bộ nội dung khóa học này.
        </p>
