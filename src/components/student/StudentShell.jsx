...
export const studentNavItems = [
  ...
  { to: '/hoc-vien/khoa-hoc-da-dang-ky', label: 'Khóa đã đăng ký', icon: 'book' },
  { to: '/hoc-vien/khoa-hoc', label: 'Lớp học của tôi', icon: 'course' },
  { to: '/hoc-vien/ho-so', label: 'Hồ sơ', icon: 'profile' },
]

const activeStudent =
  'border-sky-500/40 bg-gradient-to-r from-sky-500/15 to-cyan-500/10 text-slate-900 shadow-sm dark:border-sky-400/40 dark:from-sky-500/20 dark:to-cyan-500/10 dark:text-white dark:shadow-[0_0_24px_-4px_rgba(56,189,248,0.35)]'
    ...
    <>
      <div className="border-b border-slate-300 p-5 dark:border-white/10">
        <NavLink to="/" className="block transition-opacity hover:opacity-90" onClick={closeMobile}>
          <Logo variant="navbar" forDarkSidebar={resolvedMode === 'dark'} className="max-w-[160px]" />
        </NavLink>
        <p className="mt-3 text-[10px] font-semibold uppercase tracking-[0.22em] text-sky-700 dark:text-sky-400/70">Khu học viên</p>
              ...
              <NavLink
                ...
                className={({ isActive }) =>
                  `group flex items-center gap-3 rounded-xl border px-3 py-2.5 text-sm font-medium transition-all duration-200 ${
                    isActive ? activeStudent : inactiveStudent
                  }`
                }
              >
                {({ isActive }) => (
                  <>
                    <NavIcon
                      name={item.icon}
                      className={
                        isActive ? 'text-sky-700 dark:text-sky-200' : 'text-slate-500 group-hover:text-slate-700 dark:group-hover:text-slate-300'
                      }
                    />
            ...
            {mobileOpen && (
              <>
                ...
                <motion.aside
                  ...
                  className={`fixed inset-y-0 left-0 z-[60] flex min-h-0 w-[min(288px,92vw)] flex-col border-r border-slate-300 bg-white shadow-2xl backdrop-blur-xl dark:border-white/10 dark:bg-slate-950/98 lg:hidden ${sidebarStudent}`}
                >
                  <SidebarContent />
                </motion.aside>
              </>
                ...
                <div className="flex min-w-0 items-center gap-3">
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="hidden text-sm text-slate-600 dark:text-slate-400 lg:block">
                      Chúc bạn học tốt, <span className="font-medium text-slate-800 dark:text-slate-200">{user?.name}</span>
                    </p>
            ...
            <NavLink
              ...
              className={({ isActive }) =>
                `flex min-w-0 flex-1 flex-col items-center gap-0.5 py-2.5 text-[10px] font-medium ${
                  isActive ? 'text-sky-600 dark:text-sky-400' : 'text-slate-500 dark:text-slate-500'
                }`
              }
            >
