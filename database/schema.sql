...
-- Quet codebase: khoa hoc/mon, bai giang + lesson_details (JSONB), de thi & luot lam, tin tuc, tuyen sinh,
-- marketing leads, lop/lich/bai tap/cham diem, tien do khoa, admin activity, cai dat.
-- Best practices: identity PKs (catalog), UUID profiles = auth.users, FK indexes, RLS (auth.uid() wrapped),
-- partial indexes, timestamptz/text, snake_case, security definer helpers.
--
-- Chay: Supabase Dashboard > SQL Editor (project moi), hoac supabase db push / psql.
-- Prereq: Supabase Auth. public.profiles -> auth.users.
...
-- ---------------------------------------------------------------------------
-- Profiles (1:1 auth.users) — MUST exist before is_admin / is_teacher (SQL body is validated at create)
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- Security definer helpers (after public.profiles exists)
-- ---------------------------------------------------------------------------
...
-- Public landing teachers (data.ts imageSrc) -> avatar_url
...
-- ---------------------------------------------------------------------------
-- New Auth user â†’ profiles + student_profiles
-- ---------------------------------------------------------------------------
...
-- service_role bypasses RLS; chi dung server-side, khong lo key ra browser.
